...
function SystemSettings({ config, updateConfig, saveConfig }) {
  ...
  const restartAs = (mode) => {
    ...
    fetch("/restart")
      .then((response) => {
        if (response.ok) {
          alert(
            "System is Restarting as WiFi " +
              (mode === "client" ? "Client" : "Access Point")
          );
        } else {
  ...
  return (
    <div className="setting">
      ...
      <button id="redButton" onClick={() => restartAs("client")}>
        Restart as Client
      </button>
      <button id="redButton" onClick={() => restartAs("ap")}>
        Restart as Access Point
      </button>
    </div>
  );
}
